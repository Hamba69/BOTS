//+------------------------------------------------------------------+
//|                          Elite_Hybrid_Scalper_v6.2               |
//|           Optimized for $5-$100K | Aggressive Scalping           |
//|                   FBS BROKER OPTIMIZED WITH SPREAD COMPENSATION  |
//+------------------------------------------------------------------+
#property copyright "MIN2 - Elite Hybrid Scalper v6.2 FBS"
#property link ""
#property version "6.20"
#property description "Multi-account scalper with FBS spread compensation"
#property strict
...
//--- MICRO ACCOUNT OPTIMIZATION ($5-$20)
input group "=== MICRO ACCOUNT DOUBLING MODE ==="
input bool UseMicroAccountMode = true;            // Enable $10 account optimization
input double MicroAccountThreshold = 20.0;        // Max balance for micro mode
input double HourlyTargetPercent = 100.0;         // Target % gain per hour (doubling)
input double MicroRiskPercent = 15.0;             // Risk per trade (% - ULTRA aggressive)
input int MicroTradesPerSignal = 8;               // Orders per signal (more volume)
input int MicroMaxExposure = 100;                 // Max positions (no limits)
input double MicroLotMultiplier = 2.0;            // Lot size multiplier for micro
input bool IgnoreDailyLimits = true;              // Bypass daily limits for micro

//--- MICRO SCALPING PARAMETERS
input group "=== MICRO SCALPING SETTINGS ==="
input double MicroInstantProfitPips = 3.5;        // Instant TP (3.5 pips - spread adjusted)
input double MicroSL_Pips = 8.0;                  // Tighter SL for micro (spread safe)
input double MicroTP_Pips = 12.0;                 // Tighter TP for micro (spread safe)
input bool MicroCloseAt50Percent = false;         // Close full position at instant TP
input int MicroCheckIntervalMs = 50;              // Check every 50ms (very aggressive)

//--- FBS BROKER ADJUSTMENTS
input group "=== FBS BROKER COMPENSATION ==="
input double FBS_AvgSpreadPips = 1.8;             // FBS avg spread (EURUSD ~1.5-2.0 pips)
input double FBS_SlippagePips = 0.5;              // Expected slippage on entry
input double FBS_ExitSlippagePips = 0.3;          // Expected slippage on exit
input int FBS_MaxSlippagePoints = 30;             // Max allowed slippage (3 pips)
input bool FBS_AvoidNews = true;                  // Pause trading during high spread
input double FBS_MaxSpreadPips = 4.0;             // Max spread to allow trading
input bool FBS_UseSpreadFilter = true;            // Only trade when spread is favorable

//--- MICRO ENTRY SETTINGS
input group "=== MICRO ENTRY OPTIMIZATION ==="
input bool MicroTradeEveryBar = true;             // Trade on every M1 bar
input bool MicroIgnoreFibFilters = false;         // Reduce filter requirements
input int MicroMinFibConditions = 1;              // Min Fib conditions (relaxed)
input bool MicroIgnoreH1Trend = false;            // Ignore H1 trend filter
input bool MicroForceEntries = true;              // Force entries even in consolidation

//--- MICRO RISK MANAGEMENT OVERRIDES
input group "=== MICRO RISK OVERRIDES ==="
input double MicroMaxDailyLoss = 50.0;            // Higher daily loss tolerance (%)
input int MicroMaxConsecutiveLosses = 15;         // More losses allowed
input int MicroMaxDailyTrades = 200;              // More trades allowed
input bool MicroCompoundProfits = true;           // Compound each win immediately
...
const string LogFileName = "EliteHybrid_v62_FBS.log";
...
double accountSizeCategory = 0;
double dynamicLotMultiplier = 1.0;

// Micro mode globals
bool isMicroMode = false;
double hourlyStartBalance = 0;
datetime hourlyResetTime = 0;
double hourlyPL = 0;
int hourlyTradeCount = 0;
double microLotBase = 0.01;
double currentSpreadPips = 0;
int highSpreadCount = 0;
...
int OnInit()
{
   ...
   {
      if(balance < 100) accountSizeCategory = 0;
      else if(balance < MinAccountForStandard) accountSizeCategory = 1;
      else accountSizeCategory = 2;
   ...
   FileWriteString(logHandle, "=== ELITE HYBRID SCALPER v6.2 FBS ===\n");
   FileWriteString(logHandle, "Symbol: " + tradingSymbol + "\n");
   FileWriteString(logHandle, StringFormat("Account: $%.2f\n", balance));
   FileFlush(logHandle);
   ...
   // Initialize micro mode
   if(UseMicroAccountMode && balance <= MicroAccountThreshold)
   {
      isMicroMode = true;
      hourlyStartBalance = balance;
      hourlyResetTime = TimeCurrent();
      microLotBase = SymbolInfoDouble(tradingSymbol, SYMBOL_VOLUME_MIN);
      trade.SetDeviationInPoints(FBS_MaxSlippagePoints);
      
      FileWriteString(logHandle, "=== MICRO MODE ACTIVATED ===\n");
      FileWriteString(logHandle, StringFormat("Target: $%.2f/hour (+%.0f%%)\n", 
                      balance * (HourlyTargetPercent/100.0), HourlyTargetPercent));
      FileWriteString(logHandle, StringFormat("FBS: %.1f pips spread compensation\n", FBS_AvgSpreadPips));
      FileFlush(logHandle);
   }
   
   Print("Elite Hybrid Scalper v6.2 FBS initialized!");
...
void OnDeinit(const int reason)
{
   ...
   if(logHandle != INVALID_HANDLE)
   {
      FileWriteString(logHandle, "EA Stopped\n");
      FileFlush(logHandle);
      FileClose(logHandle);
   }
}
...
void OnTick()
{
   ...
   if(!isMicroMode || !IgnoreDailyLimits)
   {
      if(dailyLossPercent >= MaxDailyLoss) isTradingAllowed = false;
      if(consecutiveLosses >= MaxConsecutiveLosses) isTradingAllowed = false;
      if(dailyTradeCount >= MaxDailyTrades) isTradingAllowed = false;
   }
   ...
   if(UseInstantProfit)
   {
      ...
      int checkInterval = isMicroMode ? MicroCheckIntervalMs : InstantCheckIntervalMs;
      if(tickTime - lastInstantCheckTime >= (uint)checkInterval)
      {
         CheckInstantProfit();
   ...
   if(newM5Bar || (UseM1Analysis && newM1Bar))
   {
      ...
      int maxExposure = isMicroMode ? MicroMaxExposure : InpMaxExposure;
      int tradesPerSignal = isMicroMode ? MicroTradesPerSignal : InpTradesPerSignal;
      
      if(openPosCount + pendingCount + (tradesPerSignal * 2) > maxExposure) return;
...
void CheckHourlyTarget()
{
   ...
   // Spread check
   if(FBS_UseSpreadFilter && currTime - lastSpreadCheck >= 5)
   {
      ...
      currentSpreadPips = (ask - bid) / (point * 10);
      
      if(currentSpreadPips > FBS_MaxSpreadPips)
         highSpreadCount++;
      
      lastSpreadCheck = currTime;
   }
   
   // Hourly reset
   if(currTime - hourlyResetTime >= 3600)
   {
      double currentBalance = AccountInfoDouble(ACCOUNT_BALANCE);
      hourlyPL = currentBalance - hourlyStartBalance;
      double hourlyPercent = (hourlyPL / hourlyStartBalance) * 100.0;
      
      FileWriteString(logHandle, "=== HOURLY REPORT ===\n");
      FileWriteString(logHandle, StringFormat("P/L: $%.2f (%.1f%%) | Trades: %d\n",
                      hourlyPL, hourlyPercent, hourlyTradeCount));
      FileFlush(logHandle);
      
      hourlyStartBalance = currentBalance;
      hourlyResetTime = currTime;
      hourlyPL = 0;
      hourlyTradeCount = 0;
      highSpreadCount = 0;
      
      if(MicroCompoundProfits)
         dynamicLotMultiplier = MicroLotMultiplier * (currentBalance / 10.0);
   }
}
...
{
   if(!FBS_UseSpreadFilter) return true;
   ...
   return (spreadPips <= FBS_MaxSpreadPips);
}
...
{
   // Spread check
   if(isMicroMode && !IsSpreadAcceptable())
      return;
   
   // Micro aggressive mode
   if(isMicroMode && MicroTradeEveryBar && isM1Bar)
   {
      PlaceMicroEntries();
      return;
   }
   ...
   if(UseH1TrendFilter && !MicroIgnoreH1Trend)
   {
   ...
   if(UseH1TrendFilter && !AllowNeutralTrend && !MicroIgnoreH1Trend)
   {
      if(bullish && h1Bearish) return;
      if(bearish && h1Bullish) return;
   }
   
   if(UseFibFilters && !MicroIgnoreFibFilters)
   {
      int minFib = isMicroMode ? MicroMinFibConditions : MinFibConditions;
      int fibScore = CheckFibonacciConditions(bullish);
      if(fibScore < minFib) return;
   }
   
   if(UseM1Analysis && isM1Bar && !isMicroMode)
   {
      int m1Score = CheckM1Conditions(bullish);
      if(m1Score < MinM1Conditions) return;
   }
   ...
   {
      if(UseBuyStop)
         PlaceOrders(ORDER_TYPE_BUY_STOP, prevHigh + entryBufferTotal, fixedSL, fixedTP, ask, bid, minDist, expTime);
      
      if(UseBuyLimit)
         PlaceOrders(ORDER_TYPE_BUY_LIMIT, prevLow - entryBufferTotal, fixedSL, fixedTP, ask, bid, minDist, expTime);
   }
   else
   {
      if(UseSellStop)
         PlaceOrders(ORDER_TYPE_SELL_STOP, prevLow - entryBufferTotal, fixedSL, fixedTP, ask, bid, minDist, expTime);
      
      if(UseSellLimit)
         PlaceOrders(ORDER_TYPE_SELL_LIMIT, prevHigh + entryBufferTotal, fixedSL, fixedTP, ask, bid, minDist, expTime);
   }
}

//+------------------------------------------------------------------+
void PlaceMicroEntries()
{
   MqlRates m1[];
   ArraySetAsSeries(m1, true);
   if(CopyRates(tradingSymbol, PERIOD_M1, 0, 2, m1) != 2) return;
   
   bool bullish = m1[0].close > m1[0].open;
   
   double point = SymbolInfoDouble(tradingSymbol, SYMBOL_POINT);
   double totalSpreadCost = (FBS_AvgSpreadPips + FBS_SlippagePips + FBS_ExitSlippagePips) * point * 10;
   
   double microSL = MicroSL_Pips * point * 10;
   double microTP = (MicroTP_Pips + (FBS_AvgSpreadPips * 0.5)) * point * 10;
   ...
   if(bullish && UseBuyStop)
      PlaceOrders(ORDER_TYPE_BUY_STOP, m1[0].high + minDist, microSL, microTP, ask, bid, minDist, expTime);
   else if(!bullish && UseSellStop)
      PlaceOrders(ORDER_TYPE_SELL_STOP, m1[0].low - minDist, microSL, microTP, ask, bid, minDist, expTime);
                ...
                double ask, double bid, double minDist, datetime expTime)
{
   ...
   int tradesToPlace = isMicroMode ? MicroTradesPerSignal : InpTradesPerSignal;
   int maxExposure = isMicroMode ? MicroMaxExposure : InpMaxExposure;
   
   if(openPosCount + pendingCount + tradesToPlace > maxExposure)
      tradesToPlace = maxExposure - (openPosCount + pendingCount);
   ...
   if(UseInstantProfit)
   {
      double instantPips = isMicroMode ? MicroInstantProfitPips : InstantProfitPips;
      double instantPipsDist = instantPips * point * 10;
      
      if(isMicroMode)
         instantPipsDist += FBS_AvgSpreadPips * point * 10;
   ...
   if(placed > 0)
   {
      FileWriteString(logHandle, StringFormat("Placed %d orders at %.5f\n", placed, entryPrice));
      FileFlush(logHandle);
   }
}
   ...
   for(int i = ArraySize(posData) - 1; i >= 0; i--)
   {
      ...
      if(profit >= posData[i].instantTP)
      {
         if(ClosePartialOnProfit)
         {
            ...
            if(closeVol >= SymbolInfoDouble(tradingSymbol, SYMBOL_VOLUME_MIN))
            {
               if(trade.PositionClosePartial(posData[i].ticket, closeVol))
               {
                  ...
                  FileWriteString(logHandle, "Instant TP partial close\n");
                  FileFlush(logHandle);
               }
            }
         }
         else
         {
            ...
            {
               FileWriteString(logHandle, "Instant TP full close\n");
               FileFlush(logHandle);
               ArrayRemove(posData, i, 1);
            }
         }
...
double CalculateLotSize(ENUM_ORDER_TYPE orderType, double entry, double sl)
{
   ...
   double riskPercent = isMicroMode ? MicroRiskPercent : InpRiskPercent;
   int tradesPerSignal = isMicroMode ? MicroTradesPerSignal : InpTradesPerSignal;
   double lotMultiplier = isMicroMode ? MicroLotMultiplier : 1.0;
   
   double riskPerTrade = balance * riskPercent / 100.0 / tradesPerSignal;
   riskPerTrade *= dynamicLotMultiplier * lotMultiplier;
   
   if(isMicroMode && MicroCompoundProfits)
   {
      double balanceGrowthFactor = balance / hourlyStartBalance;
      riskPerTrade *= balanceGrowthFactor;
   }
   ...
   if(isMicroMode)
   {
      double spreadAdjustment = (FBS_AvgSpreadPips + FBS_SlippagePips) * point * 10;
      adjustedSL += spreadAdjustment;
   }
   
   double pointDist = MathAbs(entry - adjustedSL) / point;
   ...
   double lot = (lossPerLot > 0) ? riskPerTrade / lossPerLot : minLot;
   
   if(balance <= 15.0 && lot < minLot)
      lot = minLot;
